...
static/src/fonts.css:
	css="$$(curl --location --silent --fail "https://fonts.googleapis.com/css?family=Montserrat:200i,400,700i&display=swap")"; \
	echo "$$css" | sed 's/http.*\/\(.\+) \)/.\/fonts\/\1/' > $@; \
	mkdir -p static/src/fonts; \
	pushd static/src/fonts; \
	for c in $$(echo "$$css" | grep src | awk '{print $$2}' | sed 's/url(\(.\+\))/\1/'); do \
	    curl --location --silent --fail -O $$c; \
	done
...
tmp/help.txt: bin/$(ARCH)/onboard
	mkdir -p tmp
	bin/$(ARCH)/onboard --help 2>&1 | head -n -1 > $@
